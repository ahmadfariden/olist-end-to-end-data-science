...
- **🆕 Tabrakan istilah "High Value"/"Low Value".** Istilah ini dipakai di **dua tempat dengan definisi berbeda**:
  - **Customer Behavior dashboard** (Fase 6): hasil **K-Means k=3** pada `customer_analytics_full`, full-history, 96,096 customer. "High Value" di sini = 3,318 customer (avg spend $1,051).
  - **`value_band` di Predictive Analytics** (`mongodb_schema.md`): hasil **median split** 2-tier pada `customer_value_observation`, observation-window only, 54,738 eligible customer.

  Keduanya **TIDAK merujuk ke segment atau populasi yang sama** meski istilahnya identik — beda metode (K-Means vs median split), beda cakupan data (full-history vs observation-window), beda jumlah kategori (3-tier vs 2-tier). Jangan disamakan saat membaca tabel "Top Customers to Target" di Sheet 3 (Predictive Analytics) vs Sheet 2 (Customer Behavior).
